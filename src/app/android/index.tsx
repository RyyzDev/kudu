...
export default function HomeScreen() {
  ...
  const handleAkademikPress = async () => {
    ...
    // Jika sudah pernah login, langsung ke dashboard Akademik (Venus UI)
    if (nim && phpSessId) {
      router.push('/akademik' as any);
    } else {
      // Jika belum login, arahkan ke halaman Login sesuai OS
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#f4f4f0]">
      ...
      <View className="px-6 pt-16 pb-4 flex-row items-center justify-between border-b-2 border-black bg-white">
        <View>
          <Text className="text-4xl font-black text-black uppercase tracking-widest">KUDU - KUliah DUlu</Text>
          <Text className="text-xs font-bold text-black uppercase tracking-wider bg-[#ffde59] px-2 mt-1 self-start border border-black">
            biar ga cape pindah pindah web
          </Text>
        </View>
      </View>
      ...
      {/* Credits */}
      <View className="px-6 pb-8 pt-4 mb-8 items-center gap-3">
        {/* Traktir Kopi Button */}
        <TouchableOpacity
